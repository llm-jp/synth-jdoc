...
def main():
    ...
    def collate_fn(examples):
        ...
        # 先頭にbos tokenが2つある場合がある
        bos_token_id = processor.tokenizer.convert_tokens_to_ids(processor.tokenizer.special_tokens_map["bos_token"])
        if (batch["input_ids"][:, :2] == bos_token_id).all():
    ...
    dataset = load_dataset("<user_id>/Nano-Banana-Doc-JA", split="train")
